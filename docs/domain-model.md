...
| `worker_id`, `worker_epoch` | Assigned worker incarnation |
...
- Worker identity and epoch cannot change after assignment.
- Only the assigned current epoch may renew or report the attempt.
...
| `epoch` | New opaque incarnation identity on every restart |
...
- Only the current epoch can heartbeat, poll, drain, or report work.
...
Represents the lifecycle of one model definition on one worker epoch.

| Field | Meaning |
| --- | --- |
| `id` | Opaque residency identity |
| `model_id`, `model_revision` | Exact requested definition |
| `worker_id`, `worker_epoch` | Hosting worker incarnation |
...
- At most one active residency exists for a model revision on a worker epoch.
- A residency with active executions cannot begin unloading.
- Residency from a prior worker epoch is never considered ready.
...
An immutable declaration of runtime kinds, supported task versions, maximum parallel executions, and optional runtime-specific features. Capabilities are replaced atomically on a new worker epoch.
