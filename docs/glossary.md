...
| Model residency | A model being loaded and usable inside a worker process | M5 will prefer workers where a requested model is already ready |
...
| Runtime adapter | Code that translates Conductor’s common runtime contract into one AI runtime’s API | Future Ollama and ONNX adapters |
